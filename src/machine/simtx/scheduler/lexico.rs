...
impl SimtxScheduler for Scheduler {
    fn schedule(simulator:&mut Warp<Self>) {
        ...
        if simulator.paths.is_empty() { 
            simulator.current_path = 0xFFFFFFFF;
            ...
            //      after fusion
            let old_pc = if simulator.current_path != 0xFFFFFFFF {
                Some(simulator.paths[simulator.current_path].fetch_pc)
            } else { None };
            ...
            if fusion {
                ...
                for (k, v) in fused {
                    if let Some(pc) = old_pc {
                        if k == pc {
                            simulator.current_path = npaths.len();
                        }
                    }
            ...
            simulator.current_path = work_on[0];

            // [min pc] over only eligible threads
            for i in work_on {
                // compute the path with the minimum pc
                let ipc = simulator.paths[i].fetch_pc;
                if ipc < simulator.paths[simulator.current_path].fetch_pc {
                    simulator.current_path = i
                }
            }

            // for each not-chosen path, increment their idle time
            for i in 0..simulator.paths.len() {
                if i == simulator.current_path { simulator.paths[i].time_since_scheduled = 0 }
